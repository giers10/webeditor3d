...
interface CreateWedgeBrushCommandOptions {
  center?: Vec3;
  size?: Vec3;
  snapToGrid?: boolean;
  gridSize?: number;
...
export function createCreateWedgeBrushCommand(
  options: CreateWedgeBrushCommandOptions = {}
): EditorCommand {
  ...
  return {
    ...
    label: "Create wedge brush",
    execute(context) {
      const currentDocument = context.getDocument();

      if (currentDocument.brushes[brush.id] !== undefined) {
        throw new Error(`Wedge brush ${brush.id} already exists.`);
      }
