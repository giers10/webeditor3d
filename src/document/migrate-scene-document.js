import { createStarterMaterialRegistry } from "../materials/starter-material-library";
import { createModelInstanceCollisionSettings, createModelInstance, isModelInstanceCollisionMode, normalizeModelInstanceName } from "../assets/model-instances";
...
import { createDefaultAdvancedRenderingSettings, isAdvancedRenderingWaterReflectionMode, isBoxVolumeRenderPath, isAdvancedRenderingShadowMapSize, isAdvancedRenderingShadowType, isAdvancedRenderingToneMappingMode, isWorldBackgroundMode } from "./world-settings";
...
function expectLiteralString(value, expectedValue, label) {
    if (value !== expectedValue) {
        throw new Error(`${label} must be ${expectedValue}.`);
    }
    return expectedValue;
}
function readOptionalBoolean(value, label, fallback) {
    if (value === undefined) {
        return fallback;
    }
    return expectBoolean(value, label);
}
function readOptionalFiniteNumber(value, label, fallback) {
    if (value === undefined) {
        return fallback;
    }
    return expectFiniteNumber(value, label);
}
function readOptionalNonNegativeFiniteNumber(value, label, fallback) {
    if (value === undefined) {
        return fallback;
    }
    return expectNonNegativeFiniteNumber(value, label);
}
function readOptionalPositiveInteger(value, label, fallback) {
    if (value === undefined) {
        return fallback;
    }
...
function readOptionalPositiveIntegerWithMax(value, label, fallback, max) {
    return Math.min(readOptionalPositiveInteger(value, label, fallback), max);
}
function readOptionalAllowedValue(value, label, fallback, guard) {
    if (value === undefined) {
        return fallback;
    }
    if (!guard(value)) {
...
function readAdvancedRenderingSettings(value) {
    ...
    const ambientOcclusion = value.ambientOcclusion;
    const bloom = value.bloom;
    const toneMapping = value.toneMapping;
    const depthOfField = value.depthOfField;
    ...
    const waterReflectionMode = readOptionalAllowedValue(value.waterReflectionMode, "world.advancedRendering.waterReflectionMode", defaults.waterReflectionMode, isAdvancedRenderingWaterReflectionMode);
    return {
        ...
        ambientOcclusion: {
            enabled: readOptionalBoolean(ambientOcclusion?.enabled, "world.advancedRendering.ambientOcclusion.enabled", defaults.ambientOcclusion.enabled),
            intensity: readOptionalNonNegativeFiniteNumber(ambientOcclusion?.intensity, "world.advancedRendering.ambientOcclusion.intensity", defaults.ambientOcclusion.intensity),
            radius: readOptionalNonNegativeFiniteNumber(ambientOcclusion?.radius, "world.advancedRendering.ambientOcclusion.radius", defaults.ambientOcclusion.radius),
            samples: readOptionalPositiveInteger(ambientOcclusion?.samples, "world.advancedRendering.ambientOcclusion.samples", defaults.ambientOcclusion.samples)
        },
        bloom: {
            enabled: readOptionalBoolean(bloom?.enabled, "world.advancedRendering.bloom.enabled", defaults.bloom.enabled),
            intensity: readOptionalNonNegativeFiniteNumber(bloom?.intensity, "world.advancedRendering.bloom.intensity", defaults.bloom.intensity),
            threshold: readOptionalNonNegativeFiniteNumber(bloom?.threshold, "world.advancedRendering.bloom.threshold", defaults.bloom.threshold),
        ...
        depthOfField: {
            enabled: readOptionalBoolean(depthOfField?.enabled, "world.advancedRendering.depthOfField.enabled", defaults.depthOfField.enabled),
...
function readOptionalBrushName(value, label) {
    return normalizeBrushName(expectOptionalString(value, label));
}
function readOptionalEntityName(value, label) {
    return normalizeEntityName(expectOptionalString(value, label));
}
function expectEmptyCollection(value, label) {
    if (!isRecord(value)) {
...
function readProjectAssetBoundingBox(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readModelAssetMetadata(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readImageAssetMetadata(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    return {
...
function readAudioAssetMetadata(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    return {
        ...
        channelCount: value.channelCount === null ? null : expectPositiveFiniteNumber(value.channelCount, `${label}.channelCount`),
        sampleRateHz: value.sampleRateHz === null ? null : expectPositiveFiniteNumber(value.sampleRateHz, `${label}.sampleRateHz`),
        warnings: expectStringArray(value.warnings, `${label}.warnings`)
    };
}
function readProjectAsset(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readModelInstanceCollisionSettings(value, label) {
    if (value === undefined) {
...
function readPlayerStartColliderSettings(value, label) {
    if (value === undefined) {
    ...
    const mode = readOptionalAllowedValue(value.mode, `${label}.mode`, "capsule", (candidate) => typeof candidate === "string" && isPlayerStartColliderMode(candidate));
    return createPlayerStartColliderSettings({
        mode,
        eyeHeight: value.eyeHeight === undefined ? undefined : expectPositiveFiniteNumber(value.eyeHeight, `${label}.eyeHeight`),
        capsuleRadius: value.capsuleRadius === undefined ? undefined : expectPositiveFiniteNumber(value.capsuleRadius, `${label}.capsuleRadius`),
        capsuleHeight: value.capsuleHeight === undefined ? undefined : expectPositiveFiniteNumber(value.capsuleHeight, `${label}.capsuleHeight`),
        boxSize: value.boxSize === undefined ? undefined : readVec3(value.boxSize, `${label}.boxSize`)
    });
}
function readModelInstance(value, label, assets) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readModelInstances(value, assets) {
    if (!isRecord(value)) {
...
function readOptionalVec3(value, label, fallback) {
    if (value === undefined) {
        return {
            x: fallback.x,
            y: fallback.y,
...
function expectMaterialPattern(value, label) {
...
function readMaterialRegistry(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be a record.`);
    }
    const materials = {};
...
function readFaceUvState(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readBrushFace(value, label, materials, allowMissingUvState) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    ...
    return {
        ...
        uv: value.uv === undefined && allowMissingUvState
            ? createDefaultFaceUvState()
            : readFaceUvState(value.uv, `${label}.uv`)
    };
}
function readBoxBrushFaces(value, label, materials, allowMissingUvState) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readWorldSettings(value) {
    if (!isRecord(value)) {
...
function readPointLightEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readSpotLightEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readPlayerStartEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readSoundEmitterEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    ...
    const entity = createSoundEmitterEntity({
        ...
        name: readOptionalEntityName(value.name, `${label}.name`),
        position: readVec3(value.position, `${label}.position`),
        audioAssetId: value.audioAssetId === undefined || value.audioAssetId === null
            ? undefined
...
function readLegacySoundEmitterEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    const kind = expectLiteralString(value.kind, "soundEmitter", `${label}.kind`);
...
function readTriggerVolumeEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readTeleportTargetEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readInteractableEntity(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readEntityInstance(value, label, options) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    switch (value.kind) {
...
function readEntities(value, options) {
    if (!isRecord(value)) {
...
function readInteractionAction(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
    switch (value.type) {
...
function readInteractionLink(value, label) {
    if (!isRecord(value)) {
        throw new Error(`${label} must be an object.`);
    }
...
function readInteractionLinks(value) {
    if (!isRecord(value)) {
...
export function migrateSceneDocument(source) {
    if (!isRecord(source)) {
    ...
    if (source.version !== WATER_SURFACE_DISPLACEMENT_SCENE_DOCUMENT_VERSION && source.version !== WHITEBOX_BOX_VOLUME_SCENE_DOCUMENT_VERSION && source.version !== WHITEBOX_FLOAT_TRANSFORM_SCENE_DOCUMENT_VERSION && source.version !== WHITEBOX_GEOMETRY_SCENE_DOCUMENT_VERSION) {
