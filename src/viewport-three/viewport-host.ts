...
const PATH_JUNCTION_CANDIDATE_COLOR = 0xf4a742;
const PATH_JUNCTION_PREVIEW_COLOR = 0x7c6248;
...
const TERRAIN_GRID_RESIZE_EDGE_COLOR = 0xffef2f;
const TERRAIN_GRID_RESIZE_ARROW_COLOR = 0xffffff;
const TERRAIN_GRID_RESIZE_OUTLINE_COLOR = 0x05070c;
const TERRAIN_GRID_RESIZE_OVERLAY_OFFSET = 0.22;
const TERRAIN_GRID_RESIZE_EDGE_SEGMENTS = 32;
const TERRAIN_GRID_RESIZE_EDGE_TUBE_RADIUS = 0.16;
const TERRAIN_GRID_RESIZE_EDGE_OUTLINE_RADIUS = 0.24;
const TERRAIN_GRID_RESIZE_EDGE_HOVER_MIN_THRESHOLD = 0.18;
const TERRAIN_GRID_RESIZE_EDGE_HOVER_CELL_FACTOR = 0.35;
const TERRAIN_GRID_RESIZE_EDGE_HOVER_FOOTPRINT_FACTOR = 0.08;
const TERRAIN_GRID_RESIZE_ARROW_MIN_LENGTH = 1.6;
const TERRAIN_GRID_RESIZE_ARROW_MAX_LENGTH = 4.5;
const TERRAIN_GRID_RESIZE_ARROW_CELL_FACTOR = 2.3;
const TERRAIN_GRID_RESIZE_ARROW_SHAFT_RADIUS = 0.095;
const TERRAIN_GRID_RESIZE_ARROW_SHAFT_OUTLINE_RADIUS = 0.155;
const TERRAIN_GRID_RESIZE_ARROW_HEAD_RADIUS = 0.34;
const TERRAIN_GRID_RESIZE_ARROW_HEAD_OUTLINE_RADIUS = 0.48;
const TERRAIN_GRID_RESIZE_ARROW_HEAD_LENGTH = 0.76;
const TERRAIN_GRID_RESIZE_ARROW_HEAD_OUTLINE_LENGTH = 0.94;
const TERRAIN_GRID_RESIZE_ARROW_PICK_SHAFT_RADIUS = 0.32;
const TERRAIN_GRID_RESIZE_ARROW_PICK_HEAD_RADIUS = 0.62;
const TERRAIN_GRID_RESIZE_SIDES: TerrainGridResizeSide[] = [
  "east",
  "west",
  "north",
  "south"
...
export class ViewportHost {
  ...
  dispose() {
    ...
    this.terrainGridResizeEdgeOutline.geometry.dispose();
    this.terrainGridResizeEdgeOutline.material.dispose();
    this.terrainGridResizeEdgeTube.geometry.dispose();
  ...
  private syncSimulationLocalLights(
    ...
    for (const pointLight of runtimeScene.localLights.pointLights) {
      ...
      renderObjects.light.color.set(pointLight.colorHex);
      renderObjects.light.intensity = pointLight.intensity;
      renderObjects.light.distance = pointLight.distance;
    }

    for (const spotLight of runtimeScene.localLights.spotLights) {
      const renderObjects = this.localLightRenderObjects.get(
        spotLight.entityId
      );
  ...
  private syncSimulationLightVolumes(
    ...
    for (const lightVolume of runtimeScene.volumes.light) {
      ...
      for (const [index, derivedLight] of lightVolume.lights.entries()) {
        const light = renderObjects.lights[index];

        if (light === undefined) {
          continue;
        }

        light.color.set(lightVolume.colorHex);
        light.intensity = derivedLight.intensity;
        light.distance = derivedLight.distance;
        light.decay = derivedLight.decay;
        light.position.set(
  ...
  private syncSimulationNpcs(runtimeScene: RuntimeSceneDefinition) {
    ...
    for (const runtimeNpc of runtimeScene.npcDefinitions) {
      ...
      if (!runtimeNpc.active || !runtimeNpc.visible) {
        if (renderObjects !== undefined) {
          renderObjects.group.visible = false;
        }
        continue;
      }
  ...
  private syncSimulationInteractables(runtimeScene: RuntimeSceneDefinition) {
    for (const interactable of runtimeScene.entities.interactables) {
      const previousEnabled = this.simulationInteractableEnabledById.get(
        interactable.entityId
      );

      if (previousEnabled === interactable.interactionEnabled) {
        continue;
      }
      ...
      this.rebuildEntityMarkerForId(interactable.entityId);
    }
  }
